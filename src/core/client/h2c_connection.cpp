...
#include "h2_connection.hpp" // 需要包含 H2RequestMessage 的定义
...
bool Http2cConnection::is_usable() const {
    ...
    if (!stream_->socket().is_open()) return false;
...
asio::awaitable<void> Http2cConnection::actor_loop() {
    // --- 阶段一: 握手 ---
    SPDLOG_INFO("H2C 连接 [{}]: 开始会话.", id_);

    // --- 阶段一: 初始化 nghttp2 并发送 Preface ---
    nghttp2_session_callbacks* callbacks;
    nghttp2_session_callbacks_new(&callbacks);
    auto cb_guard = Finally([&] { nghttp2_session_callbacks_del(callbacks); });
    // ... 设置所有回调 (与 H2Connection 完全相同) ...
    ...
    // 立即发送连接前言 (Preface) 和 SETTINGS 帧
    co_await do_write();

    handshake_completed_ = true; // 在 H2C 中，发送 preface 后就可以认为“握手”完成
    SPDLOG_INFO("H2C 连接 [{}] Preface 已发送. Actor 循环开始.", id_);
    ...
    while (!is_closing_ && session_) {
        ...
        if (result.index() == 0) {
            ...
            if (ec) {
                SPDLOG_WARN("H2C 连接 [{}] 网络读取失败: {}", id_, ec.message());
                is_closing_ = true;
                break;
            }

            SPDLOG_TRACE("H2C 连接 [{}] 网络读取 {} 字节.", id_, n);

            if (nghttp2_session_mem_recv(session_, reinterpret_cast<const uint8_t*>(read_buffer_.data()), n) < 0) {
                SPDLOG_ERROR("H2C 连接 [{}] 处理网络数据失败，准备关闭连接.", id_);
                is_closing_ = true;
                break;
            }
        } else {
            ...
            if (ec) {
                SPDLOG_WARN("H2C 连接 [{}] 请求通道关闭或错误: {}", id_, ec.message());
                is_closing_ = true;
                break;
            }

            SPDLOG_DEBUG("H2C  连接 [{}] 收到新请求: {}", id_, msg.request.target());

            if (is_closing_ || remote_goaway_received_) {
                SPDLOG_WARN("H2C 连接 [{}] 已收到 GOAWAY 或正在关闭，拒绝新请求.", id_);
                msg.response_channel->try_send(asio::error::operation_aborted, HttpResponse{});
                continue;
            }

            auto stream_ctx = std::make_unique<Http2Connection::StreamContext>();
            ...
            if (stream_id < 0) {
                SPDLOG_WARN("H2C 连接 [{}] 提交请求失败: stream_id = {}", id_, stream_id);
                ...
                SPDLOG_DEBUG("H2C 连接 [{}] 成功提交请求: stream_id = {}, target = {}", id_, stream_id, msg.request.target());
                streams_.emplace(stream_id, std::move(stream_ctx));
                ++active_streams_;
                SPDLOG_DEBUG("H2C 连接 [{}] 当前活跃流数量 = {}", id_, active_streams_.load());
            }
        }

        // 在每次循环的末尾，统一处理所有待发送的数据
        SPDLOG_DEBUG("H2C [{}] actor_loop end: active_streams = {}, is_closing = {}, remote_goaway_received = {}",
