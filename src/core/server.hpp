...
namespace {
    // 辅助函数，用于打印 ALPN 协议列表，方便调试
    std::string log_alpn_protos(const unsigned char* data, unsigned int len) {
        std::string result;
        unsigned int pos = 0;
        while (pos < len) {
            unsigned char proto_len = data[pos];
            pos++;
            if (pos + proto_len > len) {
                result += "[Invalid Entry]";
                break;
            }
            result += std::string(reinterpret_cast<const char*>(data + pos), proto_len);
            result += " ";
            pos += proto_len;
        }
        return result;
    ...
    */
    Server(boost::asio::io_context& ioc, boost::asio::any_io_executor work_executor, const AizixConfig& config)
        : io_context_(ioc),
          ...
          keep_alive_ms_(config.server.keep_alive_ms) {
        // 调用一个辅助函数来完成剩下的设置
...
:
    ...
    */
    boost::asio::awaitable<void> listener() {
        ...
        for (;;) // 无限循环以持续接受连接
        {
            ...
            if (alpn == "h2") {
                ...
                auto session = Http2Session::create(tls_stream, work_executor_, router_, max_request_body_size_bytes_, keep_alive_ms_);
    ...
    boost::asio::awaitable<void> handle_plain(tcp::socket sock) const {
        try {
            ...
            for (;;) {
                ...
                timer.expires_after(is_first_request ? std::chrono::milliseconds(initial_timeout_ms_) : std::chrono::milliseconds(keep_alive_ms_)); // 根据是否是第一个请求来设置超时
    ...
    boost::asio::awaitable<void> handle_https(std::shared_ptr<boost::asio::ssl::stream<tcp::socket>> tls_stream) const {
        ...
        if (tls_stream->next_layer().is_open()) {
            ...
            SPDLOG_DEBUG("HTTPS tcp socket clos {}", ec.message());
        }
    }
    ...
    void setup_acceptor(const uint16_t port, const std::string& ip) {
        ...
        // 2. 配置 acceptor
        if (auto error_code = acceptor_.open(endpoint.protocol(), ec)) {
            throw std::runtime_error("Acceptor failed to open: " + ec.message());
        }

        if (auto error_code = acceptor_.set_option(boost::asio::socket_base::reuse_address(true), ec)) {
            // 通常这不是一个致命错误，可以只打印警告
            std::cerr << "Warning: Failed to set reuse_address option: " << ec.message() << std::endl;
            ec.clear();
        }

        if (auto error_code = acceptor_.bind(endpoint, ec)) {
            throw std::runtime_error("Failed to bind to endpoint " + ip + ":" + std::to_string(port) + ". Error: " + ec.message());
        }

        if (auto error_code = acceptor_.listen(boost::asio::socket_base::max_listen_connections, ec)) {
    ...
    */
    ...
    size_t initial_timeout_ms_ = 10 * 1000;
