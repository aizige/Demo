...
HttpSession::HttpSession(tcp::socket socket, Router& router, boost::asio::any_io_executor work_exec, const size_t body_limit, const std::chrono::milliseconds timeout, const std::chrono::milliseconds keep_alive)
...
boost::asio::awaitable<void> HttpSession::run()  {
        ...
        try {
            ...
            for (;;) {
                ...
                if (result.index() == 1) {
                    // 超时
                    if (!stopped_)
                        SPDLOG_DEBUG("HTTP/1.1 request timeout.");
                    break;
                }
                ...
                // 示例：解析完成
                if (auto [error_code, n] = std::get<0>(result); error_code) {
                    if (error_code == http::error::body_limit) {
                        // --- 子情况 2a: 请求体过大 ---
...
boost::asio::ip::tcp::endpoint HttpSession::remote_endpoint() const {
    try {
