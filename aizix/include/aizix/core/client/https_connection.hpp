...
#ifndef AIZIX_HTTPS_CONNECTION_HPP
#define AIZIX_HTTPS_CONNECTION_HPP
...
public:
    ...
    std::chrono::steady_clock::time_point get_last_used_timestamp_ms() const override{return last_used_timestamp_;}
    std::chrono::steady_clock::time_point get_ping_used_timestamp_ms() const override{return last_ping_timestamp_;}
...
private:
    ...
    /// @brief 连接最后一次被使用的时间点
    /// 由 HttpClientPool 的后台维护任务使用，以判断连接是否空闲。
    std::chrono::steady_clock::time_point last_used_timestamp_;

    /// @brief 上次PING的时间点
    /// 由 HttpClientPool 的后台维护任务使用，以判断连接多久没ping过了
    std::chrono::steady_clock::time_point last_ping_timestamp_;
};


#endif //AIZIX_HTTPS_CONNECTION_HPP